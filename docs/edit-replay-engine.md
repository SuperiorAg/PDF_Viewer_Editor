...
export function decodeTiff(
  input: Uint8Array,
): Result<TiffDecodeResult, 'tiff_decode_failed' | 'unsupported_compression' | 'multi_page'>;
```
...
async function handleFsWritePdfOps(req: FsWritePdfRequest): Promise<FsWritePdfResponse> {
  ...
  return ok({
    bytesWritten: replayResult.value.byteCount,
    newFileHash: computeFileHash(replayResult.value.newBytes),
  });
}
```
...
it('round-trips simple-text.pdf with no edits to a byte-stable output', async () => {
  ...
  if (r1.ok && r2.ok) {
    ...
    expect(
      Buffer.compare(Buffer.from(r1.value.newBytes), readGolden('simple-text-no-ops.golden.pdf')),
    ).toBe(0);
  }
});
```
