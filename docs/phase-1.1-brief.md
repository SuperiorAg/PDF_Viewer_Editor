...
If you (the user) need Phase 1.1 to run _in parallel_ with Wave 6 anyway (calendar pressure), the file-ownership boundaries still work — Phase 1.1 touches `tsconfig.renderer.json`, `LICENSE` (new), `package.json` license bump, `LICENSES.md` (Nathan), handler test files (David), one new component (Riley's F1 modal). Wave 6 touches only `docs/`. No conflict. But the signal-cleanup rationale still favors sequential.
