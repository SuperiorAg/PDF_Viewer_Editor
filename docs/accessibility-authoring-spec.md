...
| `a11y.content.non-text-tagged`               | error    | WCAG 1.1.1                                                                    | no untagged image XObject on any page                                                       | pdf.js operator-stream scan + struct tree cross-check                   |
| `a11y.content.scanned-searchable`            | error    | WCAG (general)                                                                | no page is image-only without an OCR text layer                                             | pdf.js text-layer extraction count vs page count                        |
| `a11y.behavior.javascript-no-form-actions`   | warning  | by-design (security policy §14.6)                                             | no JavaScript actions in form fields                                                        | low-level `/AA` lookup                                                  |
| `a11y.appearance.color-contrast-spot-sample` | warning  | WCAG 1.4.3 (permanently `'unevaluated'` in pdf-lib — full check needs raster) | spot-samples 3 random page regions; reports contrast ratio                                  | rasterize sample + contrast math (deferred — currently `'unevaluated'`) |

**Note (Julian Wave 11):** Rule IDs and severities above were resynchronized 2026-06-19 against the v0.8.0 code in `src/main/pdf-ops/accessibility-rules/`. Prior table drafts had drift on five IDs/severities; the engine code is authoritative because it ships and is regression-tested. The 12-rule set itself is unchanged.
