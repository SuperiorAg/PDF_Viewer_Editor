...
- Phase 7 is the **polish phase** (v0.7.0; v0.7.1–v0.7.6 are the post-roadmap point releases) — it adds no document-editing capability. It adds the auto-update client, opt-in telemetry, WCAG 2.1 AA accessibility, the i18next localization framework, macOS + Linux build config, the **comprehensive 13-section in-app Help modal** (F1, 0.7.6), and the **cursor-anchored Ctrl+wheel zoom** (0.7.6). The full design lives in [`architecture-phase-7.md`](architecture-phase-7.md) (8 locked decisions P7-L-1..P7-L-8); the a11y remediation map in [`a11y-audit.md`](a11y-audit.md); the localization design in [`i18n-strategy.md`](i18n-strategy.md). The architecture is FROZEN (P7-L-FREEZE) — extend behavior, not architecture. The 0.7.1 backlog-fix wave resolved the image-export glyph defect, multi-language OCR download, the deep modal i18n (816 keys), the annotation-layer a11y name, the better-sqlite3 Node-20 baseline (L-003), and the unsaved-work install gate. 0.7.2 stood up the real `SuperiorAg/PDF_Viewer_Editor` GitHub release feed + `release.yml` workflow. 0.7.3 shipped the native WIA scanner addon. 0.7.4 shipped HiDPI + fluid Ctrl+scroll zoom + synced dropdown. 0.7.5 shipped the new app icon. 0.7.6 layered cursor-anchored zoom + the 13-section Help modal + closed H-30.1 (real pdf-lib Combine engine — the Phase-1 stub is gone). See [Auto-update architecture](#auto-update-architecture-phase-7).
...
- **Auto-update is library-injected, not direct-imported.** `electron-updater` is loaded via a runtime `require` _inside_ `loadElectronUpdaterModule`, but that loader + `createAutoUpdateController` are STATICALLY imported into `src/main/index.ts` — the deliberate fix for the Phase-6.1 vite-tree-shake trap (see [Common pitfalls → Runtime `require()`](#runtime-require-of-in-tree-modules--vite-tree-shake--electron-30-esm-rejection)). The publish target is the real `SuperiorAg/PDF_Viewer_Editor` GitHub release feed as of 0.7.2; the controller returns the honest result of a real network call (`up-to-date` / `update-available` / a real feed error), never a fabricated "up to date". Auto-update _install_ still needs a Windows code-signing certificate. Read [Auto-update architecture](#auto-update-architecture-phase-7).
...
**Phase 5.2 closure (0.7.18 — Marcus dispatch, 2026-06-04).** Three follow-ups closed after the v0.7.13->v0.7.17 OCR-chain arc:

- **Per-page words restore on reopen.** New `ocr:listResultsByJob` channel + `OcrResultsRepoBridge.listPageResultsByJobId` bridge method ([`src/main/db-bridge.ts:listPageResultsByJobId`](../src/main/db-bridge.ts)). The bridge owns the JSON parse boundary — `safeParseWordsJson` is per-row-best-effort (one malformed `words_json` doesn't deny the rest of the document's overlay). `loadOcrResultsThunk` ([`src/client/state/thunks-phase5.ts`](../src/client/state/thunks-phase5.ts)) chains the call after `listJobs` so the slice's `setCurrentSummary({...summary, pageResults})` reducer populates `pageResultsByPage`, and the confidence overlay repaints automatically because it subscribes to that slice.
- **Standard-font factories on the OCR rasterizer.** `rasterizePageProd` ([`src/main/pdf-ops/ocr-bootstrap.ts`](../src/main/pdf-ops/ocr-bootstrap.ts)) now passes `standardFontDataUrl` / `StandardFontDataFactory` / `cMapUrl` / `cMapPacked` / `CMapReaderFactory` to `pdfjs.getDocument(...)`, reusing the SAME `resolveExportFontData()` helper the export path uses (now exported from [`src/main/export/export-bootstrap.ts`](../src/main/export/export-bootstrap.ts)). Project rule: **any pdf.js consumer in main MUST mirror the export-bootstrap font-factory wiring or text rasterizes blank.** Adds a FONT-READINESS GATE (`page.getOperatorList()` before `page.render()`) mirroring the export rasterizer's gate at [`src/main/export/pdfjs-source.ts:615`](../src/main/export/pdfjs-source.ts).
- **Rotation handling in `OcrConfidenceOverlay`.** Component accepts a `rotation: 0 | 90 | 180 | 270` prop; new `projectRectToCss(rect, pageWidthPts, pageHeightPts, scale, rotation)` helper applies the appropriate pdf-to-CSS rect projection per orientation. Call site at [`src/client/components/pdf-canvas/index.tsx`](../src/client/components/pdf-canvas/index.tsx) passes `rotation={props.page.rotation}`.

The closure means the OCR pipeline is now structurally complete end-to-end: persist (Phase 5.0), rasterize standard-font PDFs correctly (5.2 Item B), display on rotated pages (5.2 Item C), restore overlay on reopen (5.2 Item A), Save replay (Phase 5.0). The "next downstream surprise" hunt that drove v0.7.13->v0.7.17 has no known remaining layer.
...
Nine new IPC channels under two new namespaces — `ocr:*` (seven) and `scan:*` (two). All `ocr:*` channels have been LIVE since 0.5.0; **as of 0.7.3 both `scan:*` channels are LIVE on Windows** via the new native WIA addon (`native/wia-scanner/` — pure Node-API COM addon; pure N-API means a single binary works under Node + Electron, no two-ABI dance unlike better-sqlite3 L-003). On macOS / Linux the loader returns `'scanner_unavailable'`. Full contract types: [`api-contracts.md §16`](api-contracts.md). Quick reference card:
...
| `ocr:listResultsByJob`                             | **NEW in 0.7.18 (Phase 5.2).** Returns the per-page `OcrPageResult[]` for a single job — parses `words_json` from `ocr_results` at the bridge layer. Backs the "restore confidence overlay on reopen" path: `loadOcrResultsThunk` chains this after `listJobs` so reopening a previously-OCR'd doc rehydrates `pageResultsByPage` automatically. |
...
| `scan:listDevices`                                 | LIVE on Windows (0.7.3+). Enumerates WIA devices via `IWiaDevMgr2::EnumDeviceInfo`. Returns `'scanner_unavailable'` on non-Windows / addon-load-failure.                                                                                                                                                                                         |
| `scan:acquire`                                     | LIVE on Windows (0.7.3+). `IWiaTransfer::Download` + BMP→PNG normalize + pdf-lib compose. ADF multi-page → single PDF; handle chains into the OCR pipeline.                                                                                                                                                                                      |
...
### Redaction (Phase 7.4 B1)

Phase 7.4 B1 lights up Acrobat-parity redaction — the most-cited gap from Riley's [Acrobat parity audit](acrobat-parity-audit.md). Full design (the canonical reference): [`docs/phase-7.4-b1-redaction-design.md`](phase-7.4-b1-redaction-design.md) — read it before changing the engine or extending the sanitize matrix. Julian's review of the wave with the GO-with-follow-up verdict and the four follow-up findings: [`docs/code-review.md` Phase 7.4 B1 section](code-review.md). Six architectural pillars worth understanding.

**Engine path — R1 (rasterize-redact).** The engine lives at [`src/main/pdf-ops/redact-engine.ts`](../src/main/pdf-ops/redact-engine.ts). R1 means: for every page that carries at least one redaction rect, the engine rasterizes the page through the production pdfjs pipeline at a configurable DPI (default 200), draws the redaction rectangles directly into the raster via `@napi-rs/canvas`, and replaces the page in the output PDF with a fresh page that draws only the embedded raster image. The R1 / R2 / R3 decision is documented at length in the design doc §1. Picked R1 over R2 (content-stream walker) on correctness grounds: R1 makes the "original text recoverable underneath the redaction" failure mode structurally impossible (the original content stream is **not in the output PDF at all**), at the documented cost of searchability loss on redacted pages. R2 is deferred to a future phase behind a real fixture corpus + a content-stream-parser test surface; the R1 + post-Apply Run-OCR workflow is the documented v1 path until then.

**Sanitize-by-construction pattern (rebuild-from-scratch).** The engine uses `PDFDocument.create()` + `outDoc.copyPages(srcDoc, [i])` to author the output document from scratch — pass-through pages are copied via `copyPages` (which pdf-lib documents as copying only the page tree and its resources, never the catalog), and redacted pages are added via `outDoc.addPage([w, h])` + `embedPng` + `drawImage`. The source document's catalog is **never copied** into the output, so the 17 categories from the sanitize matrix (`/Info`, XMP `/Metadata`, `/Names → /JavaScript`, `/Names → /EmbeddedFiles`, `/Outlines`, `/AcroForm`, `/OCProperties`, `/StructTreeRoot` + `/MarkInfo` + `/Lang`, `/Threads`, catalog `/AA`, `/PieceInfo` + `/SpiderInfo`, per-redacted-page `/Annots`, hidden / off-page content, and so on) are dropped **by construction** — no catalog-level entry on the source is reachable from the output. The engine still calls `stripDocLevelJavaScript` on the output as belt-and-braces (two paths to safety), and saves with `useObjectStreams: false` so orphan-but-not-freed objects can't hide in object streams. See [`docs/phase-7.4-b1-redaction-design.md` §4](phase-7.4-b1-redaction-design.md) for the 17-row sanitize matrix.

> **Pattern (cite David's Phase 7.4 B1 Wave 2 takeaway, `1078669`):** sanitize-by-construction via `PDFDocument.create() + copyPages` is **structurally stronger** than load + delete-catalog-entries + save. pdf-lib emits every object that lives in `context.indirectObjects` regardless of reachability, so the load-and-delete path leaves orphan dicts (JS action bodies, removed-page annotations, deleted `/Info`) in the output xref. The rebuild path side-steps the orphan-emit class entirely. **Future destructive-mutation engines on pdf-lib documents should follow the rebuild-from-scratch pattern.** Riley's design §4.3 noted that combine's strip-by-construction property does NOT extend to a load-and-mutate redaction path; the rebuild approach extends it back. This is the project's standing engineering preference for any future content-stream-mutating op.

**IPC channel — `pdf:applyRedactions`.** One channel; the renderer previews marks client-side as an SVG overlay layer (no IPC traffic during mark editing). Contract types: `PdfApplyRedactionsRequest` / `PdfApplyRedactionsValue` / `PdfApplyRedactionsError` / `PdfApplyRedactionsResponse` in [`src/ipc/contracts.ts`](../src/ipc/contracts.ts). Channel name lives in the `Channels` const at the same file. Handler: [`src/ipc/handlers/pdf-apply-redactions.ts`](../src/ipc/handlers/pdf-apply-redactions.ts) — zod safeParse at the boundary → PAdES pre-flight → engine call → signature-audit invalidation backref → Result wrap. Preload bridge: `window.pdfApi.pdf.applyRedactions(req)` (the new method extends the existing `PdfApi.pdf` sub-namespace). Request carries a flat `redactions: RedactionRectIpc[]` list (page index on each item), an `invalidatesSignaturesConfirmed?: boolean` flag mirroring `OcrRunOnDocumentRequest`, and an optional `rasterDpi`. The error matrix maps 1:1 to renderer toasts:

| Error                         | What it means                                                        | Renderer behavior                                                                      |
| ----------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `invalid_payload`             | zod rejected the request at the boundary                             | Dev console.warn + toast "Something went wrong applying redactions"                    |
| `handle_not_found`            | Document handle is stale (the doc was closed)                        | Toast "Document is no longer open" + close sub-toolbar                                 |
| `no_redactions`               | `redactions[]` is empty (defensive — UI disables Apply when empty)   | Log + ignore                                                                           |
| `page_out_of_range`           | A mark references a page beyond `pageCount`                          | Toast "Some marks reference removed pages — refresh and try again" + clear stale marks |
| `rect_invalid`                | Negative width/height or off-page rect                               | Toast "Invalid redaction shape detected; please remark"                                |
| `signed_pdf_requires_confirm` | PAdES signatures present + `invalidatesSignaturesConfirmed !== true` | Re-open the Apply modal with the signature-invalidation paragraph                      |
| `pdf_load_failed`             | pdf-lib `PDFDocument.load` threw                                     | Toast "PDF could not be parsed for redaction"                                          |
| `rasterize_failed`            | pdfjs page render threw                                              | Toast "Page rasterization failed — see Help for diagnostics"                           |
| `engine_failed`               | Generic catch-all with `safeMessage`                                 | Toast "Redaction engine error: {safeMessage}"                                          |
| `output_too_large`            | Sanitized output exceeded the inline-bytes ceiling (200 MB)          | Toast "Redacted output exceeded 200 MB; reduce pages or DPI"                           |
| `cancelled`                   | User cancelled mid-Apply (forward-compat; not yet emitted in v1)     | Silent — restore sub-toolbar to idle                                                   |

**PAdES interaction (mirrors the OCR pattern).** Before any rasterize work, the handler loads the source via `PDFDocument.load` and calls `detectPriorPadesSignatures(probeDoc)` — the same Phase-7.2.5-hardened helper [`src/main/pdf-ops/pades-detect.ts`](../src/main/pdf-ops/pades-detect.ts) that the OCR handler uses. If signatures are present AND `invalidatesSignaturesConfirmed !== true`, the handler short-circuits with `signed_pdf_requires_confirm` and a `{ fields: signedFields }` detail payload — the engine is NOT called and no bytes change. The renderer reopens the Apply modal with the signature-invalidation paragraph; the user confirms; the renderer redispatches with `invalidatesSignaturesConfirmed: true`. On success the handler calls `signatureAuditRedaction.markInvalidatedByRedaction(docHash, signedFields)` in a try/catch (a DB hiccup must NOT fail the Apply — mirrors `ocr-run-on-document.ts` line 374). The bridge in [`src/main/db-bridge.ts`](../src/main/db-bridge.ts) wires the handler dep to the repo method. **Cross-link to 7.2.5 indirect-`/V` fix:** the redaction handler benefits from the 7.2.5 hardening for free; if a future regression breaks `detectPriorPadesSignatures` on indirect refs, the redaction PAdES gate breaks the same way — this is intentional (one detection helper, one regression surface). See the design doc §5 for the full two-step UX.

**Database schema (migration 0008).** [`migrations/0008_phase7.4_redaction_audit.sql`](../migrations/0008_phase7.4_redaction_audit.sql) is additive and forward-only:
...
One nullable timestamp column on the existing `signature_audit_log` table — no new table — with a partial index. Schema version stamp deferred to the runner (same pattern as v1–v7 migrations). The repo method `markInvalidatedByRedaction(docHash, fieldNames)` at [`src/db/repositories/signature-audit-repo.ts`](../src/db/repositories/signature-audit-repo.ts) uses a per-arity prepared-statement cache (better-sqlite3 cannot bind an array to a single `?` for IN-list, so the WHERE clause expands to N placeholders cached by arity — production case: 1–10 signed fields per doc, lots of reuse, O(1) re-prep cost). The complementary list method `listInvalidatedByRedaction(since)` powers the signature-audit panel's "Invalidated by redaction on YYYY-MM-DD" badge. **Distinction from the Phase 5 OCR backref:** `invalidated_by_ocr_job_id` is a FK to a long-running `ocr_jobs` row; redaction has no equivalent lifecycle (single synchronous IPC round-trip), so the redaction backref is a plain nullable timestamp. If a future phase introduces background redaction jobs the column upgrades cleanly to an FK without a destructive migration. See [`docs/data-models.md`](data-models.md) for the schema after migration 0008.

**Testing surface.** The engine carries 25 unit tests in [`src/main/pdf-ops/redact-engine.test.ts`](../src/main/pdf-ops/redact-engine.test.ts), gated by the non-negotiable **U-1 invariant** at line 436 — the structural proof that the engine produces a redacted page on which no text can be recovered. U-1 builds a synthetic 1-page PDF with a known text payload, runs the engine on a full-page redaction rect, then asserts via `pageContentStreamHasNoTextOps` + `pageContentStreamDrawsImage` that the output page has (a) **zero `/Font` entries in `Resources`** — no font means the page cannot carry a `Tj` / `TJ` text-show operator that would render text, and (b) **at least one `/XObject` whose `Subtype` is `/Image`** — the page draws only the raster. **This is a structural probe, not a substring search.** pdf-lib's content streams are `FlateDecode`-compressed by default; a `bytesIncludeAscii(output, 'REDACTME-NONCE-7a83b2')` probe returns false even when the token is logically still present in the compressed stream, so substring-byte probes are unreliable. The structural probe (Resources dict shape) is the right shape for asserting "page content is image-only." Cited in David's Phase 7.4 B1 Wave 2 takeaway. The U-8 .. U-15 block exercises the sanitize matrix row-by-row; U-17 / U-18 exercise the PAdES gate (signed-doc-no-confirm short-circuits without engine call; signed-doc-with-confirm calls the engine + writes the audit backref); U-22 pins the warning-aggregation contract; U-23 pins same-bytes-in → same-bytes-out determinism. The handler at [`src/ipc/handlers/pdf-apply-redactions.test.ts`](../src/ipc/handlers/pdf-apply-redactions.test.ts) carries 23 tests covering zod safeParse → error-mapping + bridge tolerance. Full count: 137 redaction-scoped tests across engine + handler + slice + component + i18n + repo + db-bridge (commit `fb954a0`).

**Follow-up: the `as any` cleanup (Julian finding 7.4.B1.1).** Riley's renderer-side dispatcher at [`src/client/state/thunks-phase7-4.ts`](../src/client/state/thunks-phase7-4.ts) lines 103–121 carries a `window.pdfApi.pdf as any` cast + an `engine_failed` feature-detect path that mentions "David Wave 2 not yet landed" — a parallel-wave coordination scar from when David's preload contract and Riley's thunk landed in the same parallel-wave window. David's preload (commit `1078669`) now exposes `applyRedactions` as a typed method on `PdfApi.pdf`, so the cast can be removed: re-import `PdfApplyRedactionsRequest` / `PdfApplyRedactionsResponse` from the IPC contract module and call `await window.pdfApi.pdf.applyRedactions(req)` directly. The feature-detect path becomes dead code (the typed call cannot return `undefined`). **Not a v0.8.0 blocker** — the cast is correct at runtime — but it should land in the next Riley cleanup wave before user-tested builds ship beyond v0.8.0. Tracked as [Julian finding 7.4.B1.1](code-review.md#phase-74-b1--redaction-review-julian-2026-06-15) (status: OPEN).
...
| --------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `update:check`              | LIVE (real feed, since 0.7.2)     | Contacts the real `SuperiorAg/PDF_Viewer_Editor` GitHub release feed. Returns `up-to-date` / `update-available` / a real network/feed error — never a fake status.                                                                                                                                                       |
| `update:download`           | LIVE (real feed, since 0.7.2)     | Download an available update (user-initiated only). Surfaces `signature_verification_failed` for unsigned bundles.                                                                                                                                                                                                       |
| `update:install`            | LIVE (gated on code-signing cert) | `quitAndInstall` with the H-29.1 unsaved-work gate (since 0.7.2). Returns `unsaved_work_blocks_install` if dirty and `confirmedDiscardUnsaved !== true`; the renderer prompts Save and install / Discard and install / Cancel. The cert is the next external dependency for the install path to actually apply a bundle. |
...
Phase 5 adds 9 IPC channels under two new namespaces plus 2 event streams. All 7 `ocr:*` channels have been LIVE end-to-end since 0.5.0; **as of 0.7.3, both `scan:*` channels are LIVE on Windows** via the native WIA COM addon (`native/wia-scanner/`). On macOS / Linux the `scan:*` channels return `'scanner_unavailable'`. Full contract types at [`api-contracts.md §16`](api-contracts.md).
...
| `scan:listDevices`                  | R → M         | `{}`                                                                        | `Result<{ devices: ScanDevice[] }, ScanListDevicesError>`                                                                       | LIVE on Windows (0.7.3+); `scanner_unavailable` on non-Windows. `IWiaDevMgr2::EnumDeviceInfo` via the pure-N-API addon in `native/wia-scanner/`.                                                 |
| `scan:acquire`                      | R → M         | `{ deviceId, resolution?, colorMode?, source?, maxPages? }`                 | `Result<{ handle, displayName, pageCount, warnings }, ScanAcquireError>`                                                        | LIVE on Windows (0.7.3+); `scanner_unavailable` elsewhere. `IWiaTransfer::Download` + ADF multi-page → single PDF; handle chains into the OCR pipeline.                                          |
...
Release is **CI-driven** since 0.7.2. The auto-update _client_ (electron-updater) is wired and the `publish` block in `electron-builder.yml` now points at the real `SuperiorAg/PDF_Viewer_Editor` GitHub release feed (`releaseType: draft` for publish safety — each release is created as a draft and a human promotes it to live). The in-app `update:check` contacts the live `latest.yml` feed and returns honest results. **Code-signing remains the next external dependency**: `electron-updater` correctly refuses to _apply_ an unsigned bundle, so the check + download paths work but the install path returns an error until a Windows code-signing certificate is provisioned.

The canonical release path is the CI workflow at [`.github/workflows/release.yml`](../.github/workflows/release.yml), triggered by pushing a `v*.*.*` tag to `main`. Steps:

1. Bump the version in `package.json` (e.g. `0.7.5` → `0.7.6`). Commit the bump alone with `chore(release): vX.Y.Z` (the per-checkpoint commit convention catches concurrent-write corruption — see CLAUDE.md project rules).
2. Update [`docs/build-report.md`](build-report.md) with the wave history if relevant.
3. Push to `main`: `git push origin main`.
4. Tag the commit + push the tag: `git tag vX.Y.Z && git push origin vX.Y.Z` — this triggers `release.yml` on `windows-latest`.
5. The workflow does: `setup-node` from `.nvmrc` (Node 20 per L-003) → `npm ci` (postinstall rebuilds better-sqlite3 to the Electron ABI) → tolerant WIA addon rebuild (`continue-on-error: true` — a missing/failing optional native module does not abort the release, and the JS loader degrades to `scanner_unavailable` at runtime) → typecheck + `electron-vite` build → `electron-builder --win --publish always` (uploads NSIS + portable + `latest.yml` to the tag's draft Release).
...
7. **Promote the draft to live:** `gh release edit vX.Y.Z --draft=false`. Until promoted, the live `latest.yml` feed does not advertise the release (an unauthenticated read against a draft returns 404), so users' updaters won't see it. After promotion, GitHub marks the highest semver as Latest and electron-updater resolves the feed correctly.

**Local-publish fallback (acceptable when CI is uncertain):** clear `ELECTRON_RUN_AS_NODE` (`Remove-Item Env:\ELECTRON_RUN_AS_NODE -ErrorAction SilentlyContinue`) → `npm run build` → `npx electron-builder --win --publish always` with `GH_TOKEN: $(gh auth token)`. Same outcome (draft release uploaded), just from the local host. Document the path taken in the build-report entry.

**Code-signing** remains the next external dependency for the install path to actually apply a bundle. `release.yml` already has the `WIN_CSC_LINK` + `WIN_CSC_KEY_PASSWORD` lines staged as commented placeholders; uncomment them once the cert is in GitHub Secrets. Until then, binaries are unsigned and trigger SmartScreen "unknown publisher" warnings on first launch (Gatekeeper on macOS, equivalent on Linux).
