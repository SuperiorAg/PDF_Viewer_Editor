...
- Wave 6 Font listing: `pdf:listEmbeddedFonts` — match. Note (amended 2026-06-19, Julian): original Wave 11 §3.4 stated `pdf:swapEmbeddedFont` had **NO handler** in `register.ts`. That was a static-review miss — re-verified with `grep -n "PdfSwapEmbeddedFont" src/ipc/register.ts` (handler at lines 1493-1498, import at line 192) and `git log -S "PdfSwapEmbeddedFont" --oneline` (introduced in `87071fa`, Wave 5 backend by David). Handler IS wired. Engine + handler + preload + UI + 129-line engine test (`font-swap-engine.test.ts` in same commit) are all live. Finding 11.3 reclassified RESOLVED-STALE; see §7.1.
...
| **11.3** | MEDIUM (resolved-stale)  | `src/ipc/register.ts:1493`                                         | **AMENDED 2026-06-19 (Julian):** Original Wave 11 finding claimed no `ipcMain.handle(Channels.PdfSwapEmbeddedFont, ...)` existed in `register.ts`. Diego's Wave 11 follow-up dispatch surfaced the wiring; Julian re-verified independently with `grep -n "PdfSwapEmbeddedFont\|swapEmbeddedFont" src/ipc/register.ts` (import at line 192, handler at lines 1493-1498) and `git log -S "PdfSwapEmbeddedFont" --oneline` (introduction commit `87071fa` — David's Wave 5 backend "B8 encryption + B18 font-swap + B20 sanitize + B21 doc-properties + link /Dest fix"). Engine + handler + preload + UI + 129-line engine test (`src/main/pdf-ops/font-swap-engine.test.ts`, also in `87071fa`) are ALL present. The original §11.3 was a static-review miss — I scanned for the channel name in §3.4 but the visual diff of the register.ts block was elided in my pass; the handler has been live since 2026-06-17 (Wave 5 backend). **No action required; no v0.8.0 blocker.** Self-improvement entry logged for the cross-check pattern (verify every "missing handler" claim with both grep AND git log -S before filing).                                                                                                                                                                                                                                                                                                        | RESOLVED-STALE (verified 2026-06-19) |
...
**Amended 2026-06-19 (Julian):** §11.3 reclassified RESOLVED-STALE (handler was already wired in commit `87071fa`, Wave 5 backend — a static-review miss). Updated tally — **Net OPEN: 1 HIGH + 2 MEDIUM + 2 LOW. Net FIXED-in-review: 1 MEDIUM + 1 LOW. Net RESOLVED-STALE: 1 MEDIUM.** With 11.3 resolved and L-007 sign-off GREEN (see §7.1 below), the sole residual driver of the YELLOW verdict is 11.1 (HIGH — R12 Acrobat-fixture regression test). See §7.1 closing for v0.8.0 ship-readiness assessment.
...
- **11.3 (MEDIUM → RESOLVED-STALE 2026-06-19)** — Originally filed as "missing `swapEmbeddedFont` IPC handler wiring." Verified by Julian on 2026-06-19 that the handler IS wired at `src/ipc/register.ts:1493` (commit `87071fa`, Wave 5 backend, David). No follow-up required. See §7.1 verification block.
...
### §7.1 L-007 Sign-off — 2026-06-19 (Julian)

**Verdict: GREEN.** All three pre-conditions stated in §7 above are satisfied against the post-Wave-7 codebase. Diego's parallel Wave 11 packaging commit `2389a2f` (`chore(packaging): Wave 11 qpdf bundling + license manifest + L-007 tool-registry lock`) shipped the lock text + ratchet + pre-commit + CI wiring; my independent verification below confirms each gate. L-007 is now in force.

**What was checked (three pre-conditions from §7):**

| #   | Condition                                                                         | Verification command                                                                           | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Lock text on disk in `.learnings/locked-instructions.md`                          | `grep -A 5 "^## L-007" .learnings/locked-instructions.md \| head -20`                          | GREEN — L-007 section lives at line 205 with all five standard sub-sections (constraint, why locked, reference implementation, enforcement, affected files, to unlock). Section closes the file at line 257. Format matches L-001..L-006 exemplars exactly.                                                                                                                                                                                                                                                                                                                                                                                             |
| 2   | Ratchet allowlist carves out the Accessibility sidebar tab (Tag PDF surface home) | inspection of `scripts/ratchet-tool-registry-coverage.mjs:469-473` (the `SURFACE_FILES` array) | GREEN — the ratchet scans **only** `src/client/components/toolbar/index.tsx`, `src/client/components/menu-bar/index.tsx`, and `src/client/components/shape-tools/shape-toolbar.tsx`. `src/client/components/accessibility-check-panel/**` (the Accessibility sidebar tab home of the Tag PDF tree editor + Accessibility Checker per ui-spec-phase-7.5 §27) is explicitly excluded from the scanned surface set. The L-007 lock text §affected-files line 249 documents this exclusion: "Accessibility sidebar tab (Tag PDF tree editor home; allowlisted carve-out per Julian's sign-off condition; Riley owns)." Carve-out present and authoritative. |
| 3   | Ratchet GREEN against the current Phase 7.5 post-Wave-7 codebase                  | `node scripts/ratchet-tool-registry-coverage.mjs ; echo "exit=$?"`                             | GREEN — `[ratchet] OK — every user-facing surface is registered, menu-mirrored, or allowlisted.` Exit code 0. Surface inventory: **67 ToolDefs** (with 139 unique i18n keys across nameKey/tooltipKey/ariaLabelKey), 177 surface occurrences across 176 unique i18n keys, **81 registered**, **37 menu-mirrored**, **58 allowlisted**, **0 GAPS**, 0 stale allowlist, 0 stale mirror map, 0 broken mirror map.                                                                                                                                                                                                                                          |

**What was confirmed (lock structural integrity):**

The lock text covers the five standard sections established by L-001..L-006 (constraint / why locked / reference implementation / enforcement / affected files / to unlock):

- **Constraint** (line 207): "Every user-facing toolbar button, menu item, shape sub-toolbar entry, and palette entry MUST be declared in `src/client/tools/registry.ts` as a `ToolDef` — OR covered by `MENU_MIRROR_MAP` (alias for an existing ToolDef on a different surface) OR by `ALLOWLIST` (intentionally inert affordance / one-of-N preset / intrinsic shortcut) in `scripts/ratchet-tool-registry-coverage.mjs`, each entry carrying a justifying `reason` comment. The scope is 'all post-Wave-10 toolbar/menu surfaces' per the principal's R14 mitigation in `docs/project-plan.md §4`. The Accessibility sidebar tab (Tag PDF tree editor's home; pure-display surface for the Accessibility Checker) is explicitly an allowlisted carve-out per Julian's L-007 sign-off condition." All four §7 sign-off conditions present in the constraint clause.
- **The rule (one-liner)**: "If a user can see it, click it, type it, or hear it announced, it lives in the registry." Crisp, matches the L-004/L-005/L-006 one-liner pattern.
- **Why locked** (line 213): cites the principal's Wave 8 override + Find-a-tool / i18n / a11y / chord drift pathology + the L-006 structural-defense analogue. Honest history.
- **Reference implementation** (lines 215-232): the canonical `ToolDef` shape with the seven-dimension contract (`id`, `nameKey`, `tooltipKey`, `ariaLabelKey`, `icon`, `shortcutId`, `menu`, `surfaces`, `enabledWhen`, `dispatch`, `searchKeywords`). Footnoted with the current inventory: 58 allowlisted + 37 menu-mirrored + 67 ToolDefs (139 unique i18n keys) + 81 registered surfaces + 0 gaps. Inventory line matches my ratchet run output exactly.
- **Enforcement** (lines 236-240): three numbered rules — (1) ratchet runs in `.husky/pre-commit` gate-2 AND in `.github/workflows/ci.yml` between Lint and Typecheck on BOTH OSes; (2) heuristic + suggested-ToolDef-shape output documented at script top; (3) ALLOWLIST entries require a `reason` comment AND principal approval. Pre-commit wire verified at `.husky/pre-commit` lines 24-41 (L-007 RATCHET block with full guidance on which remediation path to take on block). CI wire verified at `.github/workflows/ci.yml:121-122` ("L-007 ratchet — tool-registry coverage").
- **Affected files** (lines 244-252): 9 entries naming the canonical registry, the three primary surface files, find-a-tool palette consumer, the Accessibility sidebar tab (with the explicit carve-out note), the ratchet, the pre-commit, and the CI workflow. Riley-owned files vs Diego-owned files cleanly delineated.
- **To unlock** (lines 254-257): two paths — (a) soft unlock by extending ALLOWLIST with principal-approved `reason`, (b) hard unlock by replacing the rule with an even stronger mechanism (e.g. a registry-aware menu-bar cutover) demonstrated by a green CI run. Reverting to "no registry rule" is explicitly NOT a valid unlock. Same shape as L-002 / L-006 to-unlock clauses.

**Pre-commit + CI wiring confirmed:**

- `.husky/pre-commit` lines 24-41: L-007 ratchet block runs after lint-staged (gate-1) and before tsc-main (gate-3). On failure, prints three-option remediation guidance.
- `.github/workflows/ci.yml:121-122`: "L-007 ratchet — tool-registry coverage" job step. Per the comment block at lines 116-120, runs between Lint and Typecheck on BOTH OSes (`windows-2025-vs2026` and `ubuntu-latest`) because renderer surfaces are platform-independent.

**Residual concerns (NIT-level, document-only, NOT signed-off blockers):**

- **NIT 7.1.a (NIT, document-only):** the L-007 lock text's enforcement clause line 240 hand-waves on the "principal approval" mechanism for new ALLOWLIST entries. Today this is a comment-only audit trail; there is no CI-enforced require-PR-reviewer-on-ratchet-allowlist-change rule. Acceptable per the lock's own framing ("the comment is the audit trail"); flagged here so a future hardening wave can wire a CODEOWNERS gate if the allowlist starts churning.
- **NIT 7.1.b (NIT, document-only):** the L-007 affected-files line 248 says "`src/client/components/find-a-tool-palette/index.tsx` — informational consumer (Riley owns)." The palette is a consumer of the registry, not a surface; the lock correctly excludes it from `SURFACE_FILES`. No action — noted for completeness so a future agent doesn't accidentally add it to the scanner and trip itself up.
- **NIT 7.1.c (NIT, document-only):** the lock's reference-implementation block lists the ToolDef shape but does NOT pin a fixture-file path or canonical example ToolDef id. Future Riley additions might invent a non-canonical shape. Not a blocker today because the registry IS the example; flagged so a future spec hardening can add a "see e.g. `tools:run-accessibility-check`" pointer line.

None of the three NITs are blockers. Sign-off remains GREEN.

**Conclusion — v0.8.0 ship-readiness:**

With L-007 sign-off GREEN and §11.3 reclassified RESOLVED-STALE (handler was already wired in commit `87071fa` — see §3.4 amendment + §11.3 verification block), the residual driver of the original YELLOW verdict is **11.1 (HIGH — R12 Acrobat-fixture regression test gap)** ALONE. The original Wave 11 disposition stated 11.1 is a **defer-to-Wave-13-pre-cut** finding because: (a) the in-PDF discipline is structurally correct (full `/StructTreeRoot` rebuild + replace, not strip-post-hoc), (b) save-as-copy-by-default fires when `hasExistingTree === true` so the runtime data-loss path is mitigated, and (c) no real-world report of round-trip data loss yet. The 1-hour fixture-authoring + ~30-LOC test is a follow-up dispatch, not a block-cut. **Recommendation to Marcus: cut v0.8.0 with the residual 11.1 HIGH documented in `docs/build-report.md` v0.8.0 row + filed as a Diego post-v0.8.0 dispatch.** Verdict for v0.8.0: **YELLOW → effectively GREEN-with-one-honest-follow-up.** Diego may proceed with Wave 13 release ceremony.
...
4. File 11.3 (MEDIUM — missing swapEmbeddedFont handler) for Diego to land in the same Wave 11 commit OR Nathan to disclose honestly in user-guide Wave 12. **AMENDED 2026-06-19 (Julian):** 11.3 was RESOLVED-STALE on verification — handler was already wired in commit `87071fa` (Wave 5 backend). No follow-up needed; see §7.1.
5. Wait for Diego's L-007 lock + ratchet push before declaring Wave 11 complete; my §7 sign-off is the gate. **AMENDED 2026-06-19 (Julian):** L-007 sign-off GREEN; see §7.1.
