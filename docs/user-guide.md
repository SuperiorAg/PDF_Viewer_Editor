# User guide — PDF_Viewer_Editor 0.7.1 (Phase 7)

Welcome. This guide walks you through everything PDF_Viewer_Editor 0.7.1 can do, and is explicit about the parts that are still narrowly scoped. **0.7.0 closed the 7-phase roadmap; 0.7.1 is a backlog-fix point release that resolved most documented follow-ups.** Phase 7 is the polish phase — it adds no new document-editing capability, but it adds an interface-language picker, an opt-in telemetry framework, an auto-update client, keyboard + screen-reader accessibility, and macOS + Linux build configuration. The Phase 6 export surface (Word / Excel / PowerPoint / image — all six formats now produce valid output, including standard-font text in image exports), Phase 5 OCR + searchable PDFs (now with multi-language download), Phase 4 signing (visual + PAdES + audit log), Phase 3 forms (designer + mail merge), and Phase 2 page editing all carry through unchanged. For installation and the honest platform-support matrix, see the [README](../README.md#install). For the resolved-vs-deferred close-out, see [README → Roadmap status](../README.md#roadmap-status). For the upgrade story from 0.6.x, see [`build-report.md`](build-report.md) Phase 7 sections.
...
The deferred items (macOS / Linux real-hardware verification, a real auto-update publish target + signing certificate, and native scanner integration) remain open — they are blocked on resources outside the codebase, not on remaining engineering work. See [README → Roadmap status](../README.md#roadmap-status).
...
Use **Toolbar → Combine** or **File → Combine PDFs…** to merge two or more files into a new document.

The **Combine** modal lets you:

1. Click **Add file…** to open a file picker. Add as many files as you want (minimum 2).
2. Reorder the source list by dragging entries up and down.
3. Optionally specify a page range per source (e.g. "pages 3–5 only").
4. Click **Combine**.

Main loads each source via pdf-lib, concatenates the requested page ranges into a fresh document, and opens the result in the viewer as a new document called "Combined Document.pdf". You can then Save As to write it to disk; the saved bytes contain the full combine result.
...
**Phase 5.1 — DISABLED in v0.6.0.** The Tools → Scan from device menu item is visible but greyed out, with the tooltip:

> Scanning from a connected TWAIN / WIA device ships in Phase 5.1. v0.5.0 supports file-import OCR only. Use the Windows built-in Scan app, your scanner manufacturer's software, or any third-party scanner utility to produce a PDF or image, then open it here and run OCR.

Riley's Wave 19 library survey found no MIT/Apache-2.0/BSD WIA Node binding at the maturity bar this project requires (`wia`, `node-wia`, `wia-scanner`, `scanner-wia` either don't exist, are unrelated IoT SDKs, are commercial-wrappers, or are VBS-bridging hacks). The IPC contract reserves `scan:listDevices` and `scan:acquire` channel names so Phase 5.1 can add a real WIA Node addon (or build one as a custom Node-API native addon) additively, without breaking the v0.5.0 surface.

### Workaround for v0.5.0

1. Use the **Windows built-in Scan app** (Start → Search → "Scan") to scan a page to PDF or image.
2. Use your **scanner manufacturer's bundled software** (HP Smart, Epson Scan, Brother iPrint&Scan, Canon IJ Scan Utility, etc.) — most produce a multi-page PDF or a folder of images.
3. Open the produced file in PDF_Viewer_Editor (File → Open, or drag-drop).
4. Run [OCR](#running-ocr) over the imported pages.

This workflow produces exactly the same searchable-PDF output as Phase 5.1's native scanner integration will, just with an extra OS-level step.
...
### The honest placeholder state (Phase 7 trust-floor obligation #2 — see [Phase 7 trust floor](#phase-7-trust-floor--what-the-app-does-and-doesnt-promise))

When you click "Check for updates" in 0.7.0, the app reports:

> **Update channel not configured.** The auto-update client is wired and ready, but the project has not been published to a release channel yet. Updates will not download or install until a real channel is configured.

This is deliberate and honest. The app does **not** show a fake "you're up to date" — it has not checked a real feed, so it does not claim to have. It does **not** download anything, and it does **not** install anything.

When the project is published (Phase 7.1), a real GitHub release channel is configured and the client begins working with **zero code change** — at which point checking would report "up to date", "an update is available (vX.Y.Z)", or a network/feed error honestly. Even then, applying a downloaded update requires a code-signing certificate (Phase 7.1, trust-floor obligation #6); until the cert is in place, electron-updater correctly refuses to apply an unsigned bundle.

> **Future safety note.** Once a real channel is live, the "Restart and install" step must prompt before discarding unsaved edits (Julian H-29.1). This gate is unreachable today (the install button never appears while the channel is a placeholder), and closing it is a Phase 7.1 prerequisite before un-gating a real channel.
...
### "Scan from device" menu is greyed out (Phase 5.1)

Expected in v0.5.0. Native scanner integration ships in Phase 5.1. Use the Windows built-in Scan app or your scanner manufacturer's bundled software to produce a PDF or image, then open it here and run OCR. See [Scanning from a device](#scanning-from-a-device).
...
### "Update channel not configured" when I check for updates (Phase 7)

Expected in 0.7.0. The auto-update client is wired and ready, but the project has not been published to a release channel — the publish target is a placeholder. Updates will not download or install until a real channel is configured (Phase 7.1). This is the honest placeholder state, not a bug. See [Checking for updates](#checking-for-updates).
