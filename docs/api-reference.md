...
| **`scan:*` (Phase 5)** | **0** | **2** | Phase 5.1 placeholders. Both channels return `Result<never, 'not_implemented_phase_5_1'>`. Renderer pattern-matches the variant to render the disabled Scan menu tooltip. |
...
| **`update:*` (Phase 7)** | **3** | **0** | `check` / `download` / `install` + the `onProgress` event. **LIVE controller, placeholder publish feed** — every call returns the honest `update_not_configured` while the `electron-builder.yml` publish target is a PLACEHOLDER (P7-L-2). NEVER a fake "up to date". `autoDownload=false` always; default channel `manual` (no auto-check). Signature verification NOT disabled (`signature_verification_failed` for unsigned bundles). As of 0.7.1 `install` has an unsaved-work gate in source (`confirmedDiscardUnsaved` + `unsaved_work_blocks_install`); the install path is unreachable until a real channel exists, so it has zero functional impact in 0.7.1 and activates with the first build after a real channel lands (Julian H-29.1 / H-FIX.1). |
...
All three are Phase-1 stubs that validate their inputs and return `'not_implemented'`. The renderer can wire calls today; real engines ship in later phases.

### `pdf:combine` — **Stub** (Wave 2 follow-up implementation; Phase 1 currently returns stub)

Merge multiple PDFs into a new document. Runs in main via pdf-lib so the renderer stays responsive.
...
| Errors | `'invalid_source'`, `'handle_not_found'`, `'fs_read_failed'`, `'pdf_load_failed'`, `'invalid_page_range'`, `'not_implemented'` |
...
if (res.ok) console.log(`Wrote ${res.value.rowsWritten}/${res.value.totalRows} to ${res.value.outputPath}`);
```
...
const unsubscribe = api.events.onMailMergeProgress(evt => {
  console.log(`[${evt.jobId}] ${evt.phase} row ${evt.currentRow}/${evt.totalRows} (${evt.percent}%)`);
});
// later:
unsubscribe();
```
...
Phase 5 adds 9 IPC channels under two new namespaces plus 2 new event streams. All 7 `ocr:*` channels are LIVE end-to-end in 0.5.0; both `scan:*` channels return `not_implemented_phase_5_1` as Phase 5.1 placeholders. Full contract types at [`api-contracts.md §16`](api-contracts.md#16-phase-5-additions-2026-05-27-riley).
...
if (res.ok) {
  console.log('Installed:', res.value.installed.map(p => p.lang));
  console.log('Downloadable:', res.value.downloadable.map(p => p.lang));
}
```
...
### `scan:listDevices` — **Stub (Phase 5.1)**

Per `architecture-phase-5.md §7` Q-E deferral. The contract is reserved so Phase 5.1 is additive.
...
| Direction | Renderer → Main |
| Request | `{}` |
| Response | `Result<never, 'not_implemented_phase_5_1'>` |
| Errors | `'not_implemented_phase_5_1'` |

The renderer's `<ScanModal>` pattern-matches this variant to render the disabled menu tooltip and the Phase 5.1 deferral message.

### `scan:acquire` — **Stub (Phase 5.1)**

Same pattern as `scan:listDevices`. The renderer-facing request shape carries optional fields (`deviceId`, `resolution`, `colorMode`) that Phase 5.1 will validate; v0.5.0 ignores the payload and returns the stub error.

| Field | Value |
|---|---|
| Channel | `'scan:acquire'` |
| Direction | Renderer → Main |
| Request | `{ deviceId?: string; resolution?: number; colorMode?: 'bw' \| 'grayscale' \| 'color' }` |
| Response | `Result<never, 'not_implemented_phase_5_1'>` |
| Errors | `'not_implemented_phase_5_1'` |
...
try {
  ...
  else if (res.error === 'extraction_failed') showToast('Export engine not yet wired for Word (Phase 6.1).');
} finally {
  unsubscribe();
}
```
...
if (res.ok) console.log('Wrote', res.value.summary.outputBasename, res.value.summary.outputSizeBytes, 'bytes');
```
...
const unsubscribe = api.export.onProgress((event) => {
  switch (event.phase) {
    case 'extracting-text': setLabel(`Page ${event.pageIndex + 1} of ${event.totalPages}`); break;
    case 'completed': setSummary(event.summary); break;
    case 'failed': setError(event.error); break;
  }
});
// Call unsubscribe() when the consumer unmounts.
```
...
> **No secrets in Phase 7.** Like Phase 5 + 6, the Phase 7 IPC surface carries no cryptographic material. The two disciplines that matter here: **telemetry never carries PII / document content / file paths** (the `.strict()` schema is the structural enforcer), and **the update controller returns an honest `update_not_configured` rather than a fake "up to date"** when the publish target is a placeholder.

> **Channel status (read this):** `update:*` is LIVE controller, placeholder feed (every call → `update_not_configured` until a real publish target is configured — Phase 7.1). `telemetry:*` is LIVE end-to-end (in-memory only; nothing leaves the machine). `i18n:*` is LIVE.

### `update:check` — **Live controller; placeholder feed**
...
if (res.ok && res.value.status === 'not-configured')
  showNotice('Update channel not configured.');   // honest placeholder; never a fake "up to date"
```
